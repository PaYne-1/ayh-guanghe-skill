...
def test_current_source_is_v1_7_with_low_cost_runtime_files():
    assert (SKILL_ROOT / "VERSION").read_text(encoding="utf-8").strip() == "1.8.3"
    for relative in (
...
def test_v1_7_release_contains_low_cost_runner_and_no_secrets():
    archive = REPO_ROOT / "release" / "product-video-pipeline-v1.8.3.zip"
    ...
    with zipfile.ZipFile(archive) as bundle:
        ...
        assert bundle.read("product-video-pipeline/VERSION").decode("utf-8").strip() == "1.8.3"
        assert not any(
            name.endswith(".env")
