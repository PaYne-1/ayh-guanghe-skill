...
def test_task6_remediation_preserves_learning_safety_and_install_contract():
    ...
    for text in (content, workflow):
        for phrase in ("一镜到底", "固定机位", "完整双人对话口播"):
...
def test_current_source_is_v1_7_with_low_cost_runtime_files():
    assert (SKILL_ROOT / "VERSION").read_text(encoding="utf-8").strip() == "1.8.13"
    for relative in (
...
def test_v1_7_release_contains_low_cost_runner_and_no_secrets():
    archive = REPO_ROOT / "release" / "product-video-pipeline-v1.8.13.zip"
    ...
    with zipfile.ZipFile(archive) as bundle:
        ...
        assert bundle.read("product-video-pipeline/VERSION").decode("utf-8").strip() == "1.8.13"
        assert not any(
            name.endswith(".env")
...
def test_content_validator_enforces_confirmed_ayh_contract():
    ...
    valid = {
        ...
        "video_prompt": "一镜到底，固定机位，完整双人对话口播，产品保持原位，不要背景音乐。",
...
def test_content_paths_keep_deliverables_at_root_and_process_files_nested(tmp_path):
    ...
    package = {
        ...
        "video_prompt": "一镜到底，固定机位，完整双人对话口播，轮椅沿直线缓慢前进。",
        "publish_body": "以前老人总担心操作复杂，家里人每次都要陪在旁边。用了爱优护电动轮椅后，老人自己很快就能上手，平时在小区出门顺手多了，家属照顾也省心。有同样出门需求的家庭，可以了解一下爱优护电动轮椅。",
        "hashtags": ["#爱优护电动轮椅", "#ainsnbot高端智能电动轮椅", "#电动轮椅", "#老人专用电动轮椅"],
    }
...
def test_autodl_client_rejects_15_seconds_on_legacy_ten_second_workflow(tmp_path):
    client = load_script("autodl_h3.py")
