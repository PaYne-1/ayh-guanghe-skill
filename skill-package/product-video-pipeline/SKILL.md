...
- 每条项目固定为轮椅真实向前行驶、老人和一名陪护者或家属进行双人对话；只生成一个发布标题、一个封面标题、一套脚本、一张最终分镜图、一张封面图和一个视频。
...
- 每条项目在正式分镜通过后，必须把该 `2160×3840` 分镜作为 `first_frame`，再按相同尺寸独立生成一张合理尾帧作为 `last_frame`。合理尾帧保存到 `_工作文件/生成过程`，记录路径、SHA-256 和结构检查结果，不作为第二张一级分镜交付，也不得复用首帧。
- 所有新视频固定使用 `minimax_h3_lightx2v` 合理尾帧首尾帧方案。尾帧通过尺寸、主体完整度、产品结构和首尾视觉一致性检查前，不得 dry-run 或付费提交。
