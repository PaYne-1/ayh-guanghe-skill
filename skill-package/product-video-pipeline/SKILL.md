...
新策划采用`video_prompt_style: reference_wide_v1`的[多图全景口播模板](references/reference-wide-dialogue.md)：简洁正向全景描述、逐句说话人与闭嘴听众绑定。字段由助手写入策划，不增加用户启动必填项。旧策划缺少该字段时保留旧格式，不改写已绑定/已提交请求。
...
- 视频以分镜图为全程唯一画面基准，在同一画面上做局部动画：固定0–15秒、一个连续镜头、固定机位和中远景。背景、构图、人物身份与服装、产品外观和数量保持不变；只允许说话口型、自然微表情及分镜明确指定的产品运动，未指定则不额外安排动作。禁止运镜、切镜、换景、插入空镜或无关画面；尾帧延续同一构图。老人、陪护者和完整产品全程处于安全区。
